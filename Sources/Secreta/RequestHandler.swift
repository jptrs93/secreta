...
final class SecretRequestRouter: RequestHandler {
    ...
    func handle(payload: Data, completion: @escaping (Data) -> Void) {
        do {
            ...
            case "file.read":
                let request = try codec.decodeParams(FileReadRequest.self, from: envelope.params)
                handleFileRead(request: request, envelope: envelope, completion: completion)
            case "health.ping":
    ...
    private func handleFileRead(request: FileReadRequest, envelope: RequestEnvelope, completion: @escaping (Data) -> Void) {
        let response = service.readFile(request: request)
        switch response {
        case .success(let result):
            completion(encodeResult(result, requestId: envelope.requestId))
        case .failure(let error):
